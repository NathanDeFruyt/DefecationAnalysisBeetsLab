...
ui <- fluidPage(h1("Signes shitty data formatter and analyzer"),
                sidebarLayout(
                  sidebarPanel(
                    ...
                    # selectInput("control", label = 'control shiter', choices = unique(data$strain), selected = ifelse('N2' %in% unique(data$strain), 'N2', unique(data$strain)[1])),
                  ),    
...
server <- function(input, output){
  ...
  # ggsave('C:/Users/Bran/Dropbox/RProjects/Signe/defecation/data/output/202108', plot = hive, height = 27.517, width = 19.05)
  
  
  # output$expulsion.data <- downloadHandler(
  #   filename = function() {paste(dateTime(),"-explusion.data.csv", sep = "")},
  #   content = function(filename){write.csv(sumdata, file= filename, row.names = F)}
  #   )
